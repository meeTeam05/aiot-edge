# `components/ai` — On-device AQI alert (ESP32-S3)
...
| `SA_AI_SELF_TEST` | Smart-Air AI | `n` | Runs `src/ai_selftest_vectors.h` at boot and logs PASS/FAIL |
...
void      ai_set_enabled(bool enabled);           // runtime switch for the MQTT "ai_set" command (Task 1)
bool      ai_get_enabled(void);
```

- The runtime switch is not persisted: every boot starts **enabled** when built with
  `SA_ENABLE_AI=y`. While it is off, the window keeps filling, but nothing is inferred, beeped or
  published.
...
```bash
cd tools
gcc -std=c99 -Wall -I../include test_ai_input_host.c ../src/ai_input.c -o test_ai_input -lm && ./test_ai_input
```
...
1. From known windows (shape `(N, 24, 4)`, raw units, `.npy`), run
   ...
   This needs `numpy` plus `tensorflow` or `ai-edge-litert`. It writes `src/ai_selftest_vectors.h`
...
├── src/ai.c                    ai task, buzzer + MQTT, runtime switch, self-test runner
├── src/ai_input.c
├── src/ai_inference.cpp
├── src/ai_selftest_vectors.h   generated by tools/gen_selftest_vectors.py
