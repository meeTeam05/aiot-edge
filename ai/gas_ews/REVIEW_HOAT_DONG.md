...
Ngày rà soát: 2026-09-25, nhánh `feature/ai-inference`.

Phạm vi: toàn bộ luồng cảnh báo CO/NO2 trên thiết bị, gồm `sensor_task` → [`gas_ews.c`](src/gas_ews.c) → [`ai_scheduler.c`](../ai_scheduler/src/ai_scheduler.c) → MQTT/quạt/còi. Mô tả chi tiết từng thành phần nằm trong [README.md](README.md). Tài liệu này chỉ đánh giá.

Nguồn số liệu:
- Mọi mốc thời gian trong tài liệu được lấy bằng cách chạy chính `gas_ews.c` trên máy tính, qua [`ai/tools/sim_gas_ews_timeline.c`](../tools/sim_gas_ews_timeline.c) (mẫu 5 s đều đặn, không có model). Không phải ước lượng.
...
| # | Vấn đề | Mức độ |
|---|---|---|
| A | Ở cấu hình mặc định, **cảnh báo không tới được người nào**: chỉ quan sát, còi tắt, server/app chưa đọc `ai/state` | 🔴 |
...
Cảm biến (mỗi 5 s)
  ...
  → ai/state (MQTT) + quạt/còi (chỉ khi SA_AI_CONTROL_RELAY=y)
```
...
Đây đúng là cách QCVN 03:2019/BYT định nghĩa giới hạn TWA: nồng độ trung bình cho một ca làm việc 8 giờ. Trong firmware ([gas_ews.c](src/gas_ews.c), hàm `process_step`):
...
#### A. 🔴 Cảnh báo không tới người dùng ở cấu hình mặc định

Mặc định `SA_AI_CONTROL_RELAY=n`: không còi, không quạt. Cảnh báo chỉ nằm trong `ai/state`, nhưng:
- server chưa đọc topic này (INTEGRATION_REVIEW #13);
- app không hiển thị gì.

Nghĩa là **hệ thống tính ra "vượt ngưỡng" mà không ai biết**. Chế độ chỉ quan sát hợp lý cho giai đoạn thử, nhưng không phải là một hệ thống cảnh báo.

Đề xuất: tách **còi** khỏi **quạt**. Còi báo khi `level = 2` (vượt ngưỡng QCVN, chỉ dựa trên luật, không phụ thuộc model) nên được bật ngay cả trong chế độ chỉ quan sát. Còn lại: server lưu `ai/state` và app hiển thị/push thông báo.

#### B. 🔴 Tắt `device_mode` là ngừng giám sát khí

`device_mode` OFF làm dừng `sensor_task`, nên `gas_ews` không nhận mẫu nào. Bật lại thì preheat 10 phút nữa. Người dùng có thể hiểu "tắt thiết bị" là tắt quạt/đèn, không biết là tắt luôn cảnh báo khí độc.
...
Chỉ cần nền trôi thêm 0.7 ppm là sau vài giờ TWA báo "vượt ngưỡng" giả, và `level=2` sẽ bật quạt với còi dài. Mục "Trước khi bật điều khiển quạt" trong README đã yêu cầu hiệu chuẩn, nhưng firmware không ngăn được việc chạy khi chưa hiệu chuẩn.

Đề xuất: lưu cờ "đã hiệu chuẩn" (NVS, do `calibrate_no2` ghi). Chưa hiệu chuẩn thì báo `calibrated:false` trong `ai/state`, và không cho `level` của NO2 điều khiển quạt/còi.

#### F. 🟡 Mức cảnh báo sớm NO2 thực tế là "khoảng 60% ngưỡng"

Ngưỡng ngoại suy NO2 = 0.6 × STEL và ngưỡng model NO2 = 0.05 được chọn trên dữ liệu mô phỏng. Trên sự cố thật đã nhân biên độ về 0.7 × ngưỡng (không vượt), cảnh báo sớm NO2 vẫn bật ở **99%** sự cố. Như vậy "cảnh báo sớm NO2" thực chất là "NO2 đã tới khoảng 60% ngưỡng". Không sai, nhưng cần đặt tên/giải thích đúng trong app, và nên để `SA_AI_FAN_ON_LEVEL=2` cho NO2 nếu muốn quạt bớt chạy.
...
- **Quyền sở hữu quạt mất sau reboot** (từ luồng cũ, INTEGRATION_REVIEW #5): AI bật quạt, reboot, thì không ai tắt quạt.
...
|---|---|---|---|
| 1 | Build + flash, đọc 3 dòng log thời gian/stack/arena ([README.md](README.md) mục "Chi phí một lần suy luận") | mọi thứ "chưa kiểm chứng" | 1 buổi có board |
| 2 | Còi khi `level=2` kể cả chế độ chỉ quan sát; server lưu `ai/state`, app hiển thị | A | nhỏ (firmware) + vừa (server/app) |
| 3 | Giám sát khí tiếp khi `device_mode` OFF | B | nhỏ, nhưng phải kiểm tra lại logic relay/telemetry |
| 4 | Cờ hiệu chuẩn, chặn điều khiển khi chưa hiệu chuẩn | E | nhỏ |
...
gcc -std=c99 -O2 -Wall -Wextra -I../gas_ews/include test_gas_ews_host.c ../gas_ews/src/gas_ews.c -o test_gas_ews -lm && ./test_gas_ews golden
```
